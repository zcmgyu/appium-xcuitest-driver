        ...
        await retryInterval(15, 1000, async () => {
          ...
          try {
            ...
            if (err.message.includes('socket hang up')) {
              // this generally means that WDA has crashed, but also one more
              // try will allow us to know if it is completely gone
              socketHungUp = true;
            }
            if (socketHungUp && err.message.includes('ECONNREFUSED')) {
              // if we get here, we once could get the status but now get a disconnect
              // then WDA must have crashed. There is no point in retrying
              // again and again
              error = err;
              return;
            }
